    ...
    def get_historical_data(
        ...
        if interval == "1d":
            ...
            if range_end == "" or (range_end != "" and not prog.match(range_end)):
                ...
                try:
                    epoch_to = int(
                        datetime.strptime(range_end, "%Y-%m-%d %H:%M:%S").timestamp()
                    )
                except ValueError:
                    self.console.log(
                        "invalid end date (yyyy-mm-dd hh:mm:ss):", range_end
                    )
                    sys.exit()
            ...
            if range_start == "" or (range_start != "" and not prog.match(range_start)):
                if interval == "1m":
                    ...
                    epoch_from = str(
                        int(epoch_to)
                        - (((60 * minutes) * results) - ((60 * minutes) + 1))
                    )
            else:
                try:
                    epoch_from = int(
                        datetime.strptime(range_start, "%Y-%m-%d %H:%M:%S").timestamp()
                    )
                except ValueError:
                    self.console.log(
                        "invalid start date (yyyy-mm-dd hh:mm:ss):", range_start
                    )
                    sys.exit()
        ...
        if interval == "1d":
            tsidx = pd.DatetimeIndex(
                pd.to_datetime(df_data["date"]).dt.strftime("%Y-%m-%d")
            )
            df_data.set_index(tsidx, inplace=True)
            df_data = df_data.drop(columns=["date"])
        else:
            tsidx = pd.DatetimeIndex(
                pd.to_datetime(df_data["datetime"]).dt.strftime("%Y-%m-%d %H:%M:%S")
            )
            df_data.set_index(tsidx, inplace=True)
